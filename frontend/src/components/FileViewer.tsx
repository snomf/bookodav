...
import {
  ...
  CheckCircle, BookMarked, Archive, ChevronLeft, ChevronRight
} from 'lucide-react';
...
export function FileViewer({ file, onClose, getFileBlob, categories, onAssignCategory, storedMetadata }: FileViewerProps) {
  ...
  const [epubMetadata, setEpubMetadata] = useState<{ title?: string, creator?: string } | null>(null);
      ...
      try {
        ...
        if (isText) {
          ...
          book.loaded.metadata.then((meta: any) => {
            if (!cancelled) setEpubMetadata({ title: meta.title, creator: meta.creator });
          });
          setEpubBook(book);
        } else if (isCbz) {
          const buffer = await blob.arrayBuffer();
          ...
          const imageFiles = Object.keys(zip.files).filter(name =>
            ['.jpg', '.jpeg', '.png', '.webp', '.gif'].some(ext => name.toLowerCase().endsWith(ext))
          ).sort();
          ...
          if (!cancelled) setComicImages(urls);
        } else {
  ...
  useEffect(() => {
    return () => {
      ...
      comicImages.forEach(url => URL.revokeObjectURL(url));
    };
  }, [blobUrl, comicImages]);
  ...
  // EPUB Rendition
  useEffect(() => {
    if (isEpub && epubBook && epubContainerRef.current) {
        ...
        {loading ? (
          ...
          <div className="w-full h-full bg-white rounded-xl shadow-2xl overflow-hidden relative">
            ...
            <div className="absolute inset-y-0 left-0 flex items-center px-4 z-10 pointer-events-none">
              <button onClick={() => epubRendition?.prev()} className="p-3 rounded-full bg-slate-900/10 hover:bg-slate-900/20 text-slate-800 transition-all pointer-events-auto">
                <ChevronLeft size={24} />
              </button>
            </div>
            <div className="absolute inset-y-0 right-0 flex items-center px-4 z-10 pointer-events-none">
              <button onClick={() => epubRendition?.next()} className="p-3 rounded-full bg-slate-900/10 hover:bg-slate-900/20 text-slate-800 transition-all pointer-events-auto">
                <ChevronRight size={24} />
              </button>
            </div>
          </div>
        ) : isCbz && comicImages.length > 0 ? (
          <div className="w-full h-full flex flex-col items-center">
            <div className="flex-1 overflow-auto flex items-center justify-center relative w-full group">
              <img src={comicImages[currentComicPage]} alt={`Page ${currentComicPage + 1}`}
                className="max-w-full max-h-full object-contain shadow-2xl" />

              <div className="absolute inset-y-0 left-0 w-1/4 flex items-center justify-start px-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => setCurrentComicPage(p => Math.max(0, p - 1))}
                  className="p-4 rounded-full bg-black/50 text-white backdrop-blur-md">
                  <ChevronLeft size={32} />
                </button>
              </div>
              <div className="absolute inset-y-0 right-0 w-1/4 flex items-center justify-end px-4 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => setCurrentComicPage(p => Math.min(comicImages.length - 1, p + 1))}
                  className="p-4 rounded-full bg-black/50 text-white backdrop-blur-md">
                  <ChevronRight size={32} />
                </button>
              </div>
            </div>
            <div className="mt-4 flex items-center gap-4 bg-slate-900/50 px-6 py-2 rounded-full backdrop-blur-md border border-slate-700/50">
              <span className="text-xs font-medium text-slate-300">Page {currentComicPage + 1} of {comicImages.length}</span>
              <div className="flex gap-1">
                {comicImages.length <= 20 ? (
                  comicImages.map((_, i) => (
                    <button key={i} onClick={() => setCurrentComicPage(i)}
                      className={`w-1.5 h-1.5 rounded-full transition-all ${i === currentComicPage ? 'bg-amber-400 w-4' : 'bg-slate-600 hover:bg-slate-500'}`} />
                  ))
                ) : (
                  <input type="range" min="0" max={comicImages.length - 1} value={currentComicPage}
                    onChange={(e) => setCurrentComicPage(parseInt(e.target.value))}
